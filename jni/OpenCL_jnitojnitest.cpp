...
typedef void (*func_setTmpDirPath)(JNIEnv *env, jobject obj, jstring path);
typedef void (*func_setDriverDirectory)(JNIEnv *env, jobject obj, jstring path);

JNIEXPORT void JNICALL Java_com_pgroup_lib_OpenCLActivity_setTmpDirPath(
		JNIEnv* env,
		jobject obj,
		jstring path)
{
	__android_log_print(ANDROID_LOG_DEBUG, "OpenCLHook", "setTmpDirPath");

	void* dl_handle;
	func_setTmpDirPath p_setTmpDirPath;
	dlerror();
	dl_handle = dlopen("/data/data/jp.ac.nitech.oclhook/lib/libOpenCLo.so", RTLD_LAZY | RTLD_LOCAL);
	if(dl_handle) {
		__android_log_print(ANDROID_LOG_DEBUG,"OpenCLHook","load OK");
		__android_log_print(ANDROID_LOG_DEBUG,"OpenCLHook","dlopen %s\n","Load:libOpenCLo.so");
		dlerror();

		p_setTmpDirPath = (func_setTmpDirPath)dlsym(dl_handle, "Java_com_pgroup_lib_OpenCLActivity_setTmpDirPath");
		__android_log_print(ANDROID_LOG_DEBUG,"OpenCLHook","dlerrror : %s\n",dlerror());
		if(dlerror() == NULL) {
			p_setTmpDirPath(env,obj,path);
			__android_log_print(ANDROID_LOG_DEBUG,"OpenCLHook","setTmpDirPath()!");
		}

		dlclose(dl_handle);
	} else {
		__android_log_print(ANDROID_LOG_DEBUG,"OpenCLHook","dlerror : %s\n",dlerror());
	}
}

JNIEXPORT void JNICALL Java_com_pgroup_lib_OpenCLActivity_setDriverDirectory(
		JNIEnv* env,
		jobject obj,
		jstring path)
{
	__android_log_print(ANDROID_LOG_DEBUG, "OpenCLHook", "setDriverDirectory");

	void* dl_handle;
	func_setDriverDirectory p_setDriverDirectory;
	dlerror();
	dl_handle = dlopen("/data/data/jp.ac.nitech.oclhook/lib/libOpenCLo.so", RTLD_LAZY | RTLD_LOCAL);
	if(dl_handle) {
		__android_log_print(ANDROID_LOG_DEBUG,"OpenCLHook","load OK");
		...
		p_setDriverDirectory = (func_setDriverDirectory)dlsym(dl_handle, "Java_com_pgroup_lib_OpenCLActivity_setDriverDirectory");
		__android_log_print(ANDROID_LOG_DEBUG,"OpenCLHook","dlerrror : %s\n",dlerror());
		if(dlerror() == NULL) {
			p_setDriverDirectory(env,obj,path);
			__android_log_print(ANDROID_LOG_DEBUG,"OpenCLHook","setDriverDirectory()!");
		}
